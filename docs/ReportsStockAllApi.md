# moysklad_remap_12_sdk.ReportsStockAllApi

All URIs are relative to *https://api.moysklad.ru/api/remap/1.2*

Method | HTTP request | Description
------------- | ------------- | -------------
[**get_stock_all**](ReportsStockAllApi.md#get_stock_all) | **GET** /report/stock/all | Получить Расширенный отчет об остатках


# **get_stock_all**
> StockAllList get_stock_all(limit=limit, offset=offset, filter=filter, order=order, group_by=group_by, include_related=include_related, accept=accept, accept_encoding=accept_encoding)

Получить Расширенный отчет об остатках

Запрос Расширенного отчета об остатках по всей номенклатуре учетной записи.
...
with moysklad_remap_12_sdk.ApiClient(configuration) as api_client:
    ...
    api_instance = moysklad_remap_12_sdk.ReportsStockAllApi(api_client)
    ...
    include_related = True # bool | Вывод остатков по модификациям и партиям товаров. Обязательное условие применения — наличие фильтрации по товарам или модификациям или их комбинации. При использовании параметра устанавливается группировка `groupBy=consignment`, переданные значения `groupBy` игнорируются.  (optional)
    ...
    try:
        # Получить Расширенный отчет об остатках
        api_response = api_instance.get_stock_all(limit=limit, offset=offset, filter=filter, order=order, group_by=group_by, include_related=include_related, accept=accept, accept_encoding=accept_encoding)
        print("The response of ReportsStockAllApi->get_stock_all:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling ReportsStockAllApi->get_stock_all: %s\n" % e)
```
 ...
 **include_related** | **bool**| Вывод остатков по модификациям и партиям товаров. Обязательное условие применения — наличие фильтрации по товарам или модификациям или их комбинации. При использовании параметра устанавливается группировка &#x60;groupBy&#x3D;consignment&#x60;, переданные значения &#x60;groupBy&#x60; игнорируются.  | [optional] 
 **accept** | **str**|  | [optional] [default to application/json;charset&#x3D;utf-8]
 **accept_encoding** | **str**|  | [optional] [default to &#39;gzip, deflate, br&#39;]

### Return type

[**StockAllList**](StockAllList.md)
