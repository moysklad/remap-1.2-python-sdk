# moysklad_remap_12_sdk.UserSettingsApi

All URIs are relative to *https://api.moysklad.ru/api/remap/1.2*

Method | HTTP request | Description
------------- | ------------- | -------------
[**get_user_settings**](UserSettingsApi.md#get_user_settings) | **GET** /context/usersettings | Получить настройки пользователя
[**update_user_settings**](UserSettingsApi.md#update_user_settings) | **PUT** /context/usersettings | Изменить настройки пользователя


# **get_user_settings**
> UserSettings get_user_settings(accept=accept, accept_encoding=accept_encoding)
...
with moysklad_remap_12_sdk.ApiClient(configuration) as api_client:
    ...
    api_instance = moysklad_remap_12_sdk.UserSettingsApi(api_client)
    ...
    try:
        # Получить настройки пользователя
        api_response = api_instance.get_user_settings(accept=accept, accept_encoding=accept_encoding)
...
> UserSettings update_user_settings(user_settings, accept=accept, accept_encoding=accept_encoding, content_type=content_type)
...
with moysklad_remap_12_sdk.ApiClient(configuration) as api_client:
    # Create an instance of the API class
    api_instance = moysklad_remap_12_sdk.UserSettingsApi(api_client)
    user_settings = moysklad_remap_12_sdk.UserSettings() # UserSettings | 
    accept = application/json;charset=utf-8 # str |  (optional) (default to application/json;charset=utf-8)
    accept_encoding = 'gzip, deflate, br' # str |  (optional) (default to 'gzip, deflate, br')
    ...
    try:
        # Изменить настройки пользователя
        api_response = api_instance.update_user_settings(user_settings, accept=accept, accept_encoding=accept_encoding, content_type=content_type)
        print("The response of UserSettingsApi->update_user_settings:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling UserSettingsApi->update_user_settings: %s\n" % e)
```
...
| Status code | Description | Response headers |
|-------------|-------------|------------------|
**200** | Настройки пользователя успешно обновлены |  -  |
**0** | Ошибка запроса (тело — объект с полем errors) |  -  |
