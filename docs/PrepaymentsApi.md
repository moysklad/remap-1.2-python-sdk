# moysklad_remap_12_sdk.PrepaymentsApi

All URIs are relative to *https://api.moysklad.ru/api/remap/1.2*

Method | HTTP request | Description
------------- | ------------- | -------------
[**create_prepayment_metadata_attribute**](PrepaymentsApi.md#create_prepayment_metadata_attribute) | **POST** /entity/prepayment/metadata/attributes | Создать доп. поле Предоплаты


# **create_prepayment_metadata_attribute**
> AttributeMetaInfo create_prepayment_metadata_attribute(attribute_meta_info, accept=accept, accept_encoding=accept_encoding, content_type=content_type)
...
with moysklad_remap_12_sdk.ApiClient(configuration) as api_client:
    ...
    api_instance = moysklad_remap_12_sdk.PrepaymentsApi(api_client)
    ...
    try:
        # Создать доп. поле Предоплаты
        api_response = api_instance.create_prepayment_metadata_attribute(attribute_meta_info, accept=accept, accept_encoding=accept_encoding, content_type=content_type)
        print("The response of PrepaymentsApi->create_prepayment_metadata_attribute:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling PrepaymentsApi->create_prepayment_metadata_attribute: %s\n" % e)
```
