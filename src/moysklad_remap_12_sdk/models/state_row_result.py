...
STATEROWRESULT_ONE_OF_SCHEMAS = ["Errors", "State"]

class StateRowResult(BaseModel):
    """
    StateRowResult
    """
    # data type: State
    ...
    actual_instance: Optional[Union[Errors, State]] = None
    one_of_schemas: Set[str] = { "Errors", "State" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = StateRowResult.model_construct()
        error_messages = []
        match = 0
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in StateRowResult with oneOf schemas: Errors, State. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in StateRowResult with oneOf schemas: Errors, State. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into StateRowResult with oneOf schemas: Errors, State. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into StateRowResult with oneOf schemas: Errors, State. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], Errors, State]]:
