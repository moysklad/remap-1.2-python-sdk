...
STATEROWRESULT_ONE_OF_SCHEMAS = ["Error", "State"]
...
class StateRowResult(BaseModel):
    ...
    actual_instance: Optional[Union[Error, State]] = None
    one_of_schemas: Set[str] = { "Error", "State" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in StateRowResult with oneOf schemas: Error, State. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in StateRowResult with oneOf schemas: Error, State. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into StateRowResult with oneOf schemas: Error, State. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into StateRowResult with oneOf schemas: Error, State. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], Error, State]]:
