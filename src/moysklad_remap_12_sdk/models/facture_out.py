# coding: utf-8

"""
    МойСклад JSON API

    API для манипуляции с сущностями и создания отчетов в онлайн-сервисе МойСклад.  ## Аутентификация  МойСклад поддерживает аутентификацию по протоколу Basic Auth и с использованием токена доступа: - Basic Auth: заголовок `Authorization` со значением пары `логин:пароль`, закодированным в Base64 - Bearer Token: заголовок `Authorization` со значением `Bearer <Access-Token>`  ## Ограничения  - Не более 45 запросов за 3 секундный период от аккаунта - Не более 5 параллельных запросов от одного пользователя   - Не более 20 параллельных запросов от аккаунта - Не более 20 Мб данных в одном запросе - Максимум 1000 элементов в массиве - Обязательное использование сжатия gzip 

    The version of the OpenAPI document: 1.0.0
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


from __future__ import annotations
import pprint
import re  # noqa: F401
import json

from pydantic import BaseModel, ConfigDict, Field, StrictBool, StrictFloat, StrictInt, StrictStr
from typing import Any, ClassVar, Dict, List, Optional, Union
from typing_extensions import Annotated
from moysklad_remap_12_sdk.models.agent import Agent
from moysklad_remap_12_sdk.models.attribute_abstract import AttributeAbstract
from moysklad_remap_12_sdk.models.contract import Contract
from moysklad_remap_12_sdk.models.employee import Employee
from moysklad_remap_12_sdk.models.file_list import FileList
from moysklad_remap_12_sdk.models.group import Group
from moysklad_remap_12_sdk.models.meta import Meta
from moysklad_remap_12_sdk.models.organization import Organization
from moysklad_remap_12_sdk.models.state import State
from typing import Optional, Set
from typing_extensions import Self

class FactureOut(BaseModel):
    """
    Счет-фактура выданный (factureout)
    """ # noqa: E501
    meta: Optional[Meta] = None
    id: Optional[StrictStr] = Field(default=None, description="ID Счета-фактуры выданного")
    account_id: Optional[StrictStr] = Field(default=None, description="ID учетной записи", alias="accountId")
    agent: Optional[Agent] = Field(default=None, description="Метаданные контрагента или юрлица")
    applicable: Optional[StrictBool] = Field(default=None, description="Отметка о проведении")
    attributes: Optional[List[AttributeAbstract]] = Field(default=None, description="Коллекция метаданных доп. полей")
    code: Optional[Annotated[str, Field(strict=True, max_length=255)]] = Field(default=None, description="Код Счета-фактуры выданного")
    contract: Optional[Contract] = Field(default=None, description="Метаданные договора")
    created: Optional[StrictStr] = Field(default=None, description="Дата создания")
    deleted: Optional[StrictStr] = Field(default=None, description="Момент последнего удаления Счета-фактуры выданного")
    description: Optional[Annotated[str, Field(strict=True, max_length=4096)]] = Field(default=None, description="Комментарий Счета-фактуры выданного")
    external_code: Optional[Annotated[str, Field(strict=True, max_length=255)]] = Field(default=None, description="Внешний код Счета-фактуры выданного", alias="externalCode")
    files: Optional[FileList] = Field(default=None, description="Метаданные массива файлов")
    group: Optional[Group] = None
    moment: Optional[StrictStr] = Field(default=None, description="Дата документа")
    name: Optional[Annotated[str, Field(strict=True, max_length=255)]] = Field(default=None, description="Наименование Счета-фактуры выданного")
    organization: Optional[Organization] = None
    owner: Optional[Employee] = Field(default=None, description="Владелец (Сотрудник)")
    printed: Optional[StrictBool] = Field(default=None, description="Напечатан ли документ")
    published: Optional[StrictBool] = Field(default=None, description="Опубликован ли документ")
    rate: Optional[CurrencyRate] = Field(default=None, description="""Валюта документа. Если значение курса валюты не указано, используется курс из справочника валют. """, alias="rate")
    shared: Optional[StrictBool] = Field(default=None, description="Общий доступ")
    state: Optional[State] = Field(default=None, description="Метаданные статуса Счета-фактуры выданного")
    state_contract_id: Optional[Annotated[str, Field(strict=True, max_length=255)]] = Field(default=None, description="Идентификатор государственного контракта, договора (соглашения)", alias="stateContractId")
    sum: Optional[Union[StrictFloat, StrictInt]] = Field(default=None, description="Сумма Счета-фактуры выданного в копейках")
    sync_id: Optional[StrictStr] = Field(default=None, description="ID синхронизации", alias="syncId")
    updated: Optional[StrictStr] = Field(default=None, description="Момент последнего обновления Счета-фактуры выданного")
    advance_payment_vat: Optional[Union[StrictFloat, StrictInt]] = Field(default=None, description="Ставка НДС для авансового платежа (в процентах). Доступно только для счетов-фактур с основаниями-платежами. ", alias="advancePaymentVat")
    payment_purpose: Optional[Annotated[str, Field(strict=True, max_length=255)]] = Field(default=None, description="Назначение платежа. Доступно только для счетов-фактур с основаниями-платежами. ", alias="paymentPurpose")
    vat_sum: Optional[Union[StrictFloat, StrictInt]] = Field(default=None, description="Сумма включая НДС. Доступно только для счетов-фактур с основаниями-платежами. ", alias="vatSum")
    demands: Optional[List[Demand]] = Field(default=None, description="Связанные отгрузки (метаданные)")
    payments: Optional[List[PaymentIn]] = Field(default=None, description="Связанные входящие платежи (метаданные)")
    returns: Optional[List[PurchaseReturn]] = Field(default=None, description="Связанные возвраты поставщикам (метаданные)")
    consignee: Optional[Agent] = Field(default=None, description="Грузополучатель (контрагент или юрлицо)")
    payment_number: Optional[Annotated[str, Field(strict=True, max_length=255)]] = Field(default=None, description="Название платежного документа", alias="paymentNumber")
    payment_date: Optional[StrictStr] = Field(default=None, description="Дата платежного документа", alias="paymentDate")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["meta", "id", "accountId", "agent", "applicable", "attributes", "code", "contract", "created", "deleted", "description", "externalCode", "files", "group", "moment", "name", "organization", "owner", "printed", "published", "rate", "shared", "state", "stateContractId", "sum", "syncId", "updated", "advancePaymentVat", "paymentPurpose", "vatSum", "demands", "payments", "returns", "consignee", "paymentNumber", "paymentDate"]

    model_config = ConfigDict(
        extra="allow",
        arbitrary_types_allowed=True,
        populate_by_name=True,
        validate_assignment=True,
        protected_namespaces=(),
    )

    @classmethod
    def create_with_meta(
        cls,
        id: str,
    ) -> Self:
        """Create a reference with ``meta`` populated for this entity."""
        from moysklad_remap_12_sdk.configuration import Configuration
        from moysklad_remap_12_sdk.models.meta import Meta

        href = Configuration.get_default().host + "/entity" + "/factureout" + "/" + id
        return cls(
            id=id,
            meta=Meta(
                href=href,
                type="factureout",
                media_type="application/json",
            ),
        )


    def to_str(self) -> str:
        """Returns the string representation of the model using alias"""
        return pprint.pformat(self.model_dump(by_alias=True))

    def to_json(self) -> str:
        """Returns the JSON representation of the model using alias"""
        return json.dumps(self.to_dict())

    @classmethod
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of FactureOut from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    def to_dict(self) -> Dict[str, Any]:
        """Return the dictionary representation of the model using alias.

        `None` is only added for nullable fields set at initialization.
        OpenAPI `readOnly` response fields are retained.
        """
        excluded_fields: Set[str] = {
            "additional_properties",
        }

        _dict = self.model_dump(
            by_alias=True,
            exclude=excluded_fields,
            exclude_none=True,
        )
        if self.meta:
            _dict['meta'] = self.meta.to_dict()
        if self.agent:
            _dict['agent'] = self.agent.to_dict()
        _items = []
        if self.attributes:
            for _item_attributes in self.attributes:
                if _item_attributes:
                    _items.append(_item_attributes.to_dict())
            _dict['attributes'] = _items
        if self.contract:
            _dict['contract'] = self.contract.to_dict()
        if self.files:
            _dict['files'] = self.files.to_dict()
        if self.group:
            _dict['group'] = self.group.to_dict()
        if self.organization:
            _dict['organization'] = self.organization.to_dict()
        if self.owner:
            _dict['owner'] = self.owner.to_dict()
        if self.rate:
            _dict['rate'] = self.rate.to_dict()
        if self.state:
            _dict['state'] = self.state.to_dict()
        _items = []
        if self.demands:
            for _item_demands in self.demands:
                if _item_demands:
                    _items.append(_item_demands.to_dict())
            _dict['demands'] = _items
        _items = []
        if self.payments:
            for _item_payments in self.payments:
                if _item_payments:
                    _items.append(_item_payments.to_dict())
            _dict['payments'] = _items
        _items = []
        if self.returns:
            for _item_returns in self.returns:
                if _item_returns:
                    _items.append(_item_returns.to_dict())
            _dict['returns'] = _items
        if self.consignee:
            _dict['consignee'] = self.consignee.to_dict()
        if self.additional_properties is not None:
            for _key, _value in self.additional_properties.items():
                _dict[_key] = _value

        if self.attributes is None and "attributes" in self.model_fields_set:
            _dict['attributes'] = None

        if self.contract is None and "contract" in self.model_fields_set:
            _dict['contract'] = None

        if self.description is None and "description" in self.model_fields_set:
            _dict['description'] = None

        if self.files is None and "files" in self.model_fields_set:
            _dict['files'] = None

        if self.owner is None and "owner" in self.model_fields_set:
            _dict['owner'] = None

        if self.state is None and "state" in self.model_fields_set:
            _dict['state'] = None

        if self.demands is None and "demands" in self.model_fields_set:
            _dict['demands'] = None

        if self.payments is None and "payments" in self.model_fields_set:
            _dict['payments'] = None

        if self.returns is None and "returns" in self.model_fields_set:
            _dict['returns'] = None

        if self.consignee is None and "consignee" in self.model_fields_set:
            _dict['consignee'] = None

        return _dict

    @classmethod
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of FactureOut from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return cls.model_validate(obj)

        _data = dict(obj)
        _data.update({
            "meta": Meta.from_dict(obj["meta"]) if obj.get("meta") is not None else None,
            "id": obj.get("id"),
            "accountId": obj.get("accountId"),
            "agent": Agent.from_dict(obj["agent"]) if obj.get("agent") is not None else None,
            "applicable": obj.get("applicable"),
            "attributes": [AttributeAbstract.from_dict(_item) for _item in obj["attributes"]] if obj.get("attributes") is not None else None,
            "code": obj.get("code"),
            "contract": Contract.from_dict(obj["contract"]) if obj.get("contract") is not None else None,
            "created": obj.get("created"),
            "deleted": obj.get("deleted"),
            "description": obj.get("description"),
            "externalCode": obj.get("externalCode"),
            "files": FileList.from_dict(obj["files"]) if obj.get("files") is not None else None,
            "group": Group.from_dict(obj["group"]) if obj.get("group") is not None else None,
            "moment": obj.get("moment"),
            "name": obj.get("name"),
            "organization": Organization.from_dict(obj["organization"]) if obj.get("organization") is not None else None,
            "owner": Employee.from_dict(obj["owner"]) if obj.get("owner") is not None else None,
            "printed": obj.get("printed"),
            "published": obj.get("published"),
            "rate": CurrencyRate.from_dict(obj["rate"]) if obj.get("rate") is not None else None,
            "shared": obj.get("shared"),
            "state": State.from_dict(obj["state"]) if obj.get("state") is not None else None,
            "stateContractId": obj.get("stateContractId"),
            "sum": obj.get("sum"),
            "syncId": obj.get("syncId"),
            "updated": obj.get("updated"),
            "advancePaymentVat": obj.get("advancePaymentVat"),
            "paymentPurpose": obj.get("paymentPurpose"),
            "vatSum": obj.get("vatSum"),
            "demands": [Demand.from_dict(_item) for _item in obj["demands"]] if obj.get("demands") is not None else None,
            "payments": [PaymentIn.from_dict(_item) for _item in obj["payments"]] if obj.get("payments") is not None else None,
            "returns": [PurchaseReturn.from_dict(_item) for _item in obj["returns"]] if obj.get("returns") is not None else None,
            "consignee": Agent.from_dict(obj["consignee"]) if obj.get("consignee") is not None else None,
            "paymentNumber": obj.get("paymentNumber"),
            "paymentDate": obj.get("paymentDate")
        })
        _obj = cls.model_validate(_data)
        for _key in obj.keys():
            if _key not in cls.__properties:
                _obj.additional_properties[_key] = obj.get(_key)

        return _obj

from moysklad_remap_12_sdk.models.demand import Demand
from moysklad_remap_12_sdk.models.payment_in import PaymentIn
from moysklad_remap_12_sdk.models.purchase_return import PurchaseReturn
# TODO: Rewrite to not use raise_errors
FactureOut.model_rebuild(raise_errors=False)


from moysklad_remap_12_sdk.models.meta import Meta







from moysklad_remap_12_sdk.models.agent import Agent






from moysklad_remap_12_sdk.models.attribute_abstract import AttributeAbstract




from moysklad_remap_12_sdk.models.contract import Contract











from moysklad_remap_12_sdk.models.file_list import FileList



from moysklad_remap_12_sdk.models.group import Group







from moysklad_remap_12_sdk.models.organization import Organization



from moysklad_remap_12_sdk.models.employee import Employee







from moysklad_remap_12_sdk.models.currency_rate import CurrencyRate





from moysklad_remap_12_sdk.models.state import State


















from moysklad_remap_12_sdk.models.demand import Demand



from moysklad_remap_12_sdk.models.payment_in import PaymentIn



from moysklad_remap_12_sdk.models.purchase_return import PurchaseReturn


from moysklad_remap_12_sdk.models.agent import Agent







