...
class ProcessingProcessPosition(EntityWithMeta):
    """
    Позиция Техпроцесса
    """ # noqa: E501
    ...
    processingstage: Optional[ProcessingStage] = None
    next_positions: Optional[List[ProcessingProcessPositionNextPositionsInner]] = Field(default=None, description="Метаданные следующих позиций позиции Техпроцесса", alias="nextPositions")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["meta", "id", "accountId", "processingstage", "nextPositions"]
    ...
    def create_with_meta(
        ...
        href = Configuration.get_default().host + "/entity" + "/processingprocess" + "/" + parentId + "/positions" + "/" + id
        return cls(
            id=id,
            meta=Meta(
                href=href,
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.processingstage:
            _dict['processingstage'] = self.processingstage.to_dict()
        _items = []
        if self.next_positions:
            for _item_next_positions in self.next_positions:
                if _item_next_positions:
                    _items.append(_item_next_positions.to_dict())
            _dict['nextPositions'] = _items
        if self.additional_properties is not None:
            for _key, _value in self.additional_properties.items():
                _dict[_key] = _value

        if self.next_positions is None and "next_positions" in self.model_fields_set:
            _dict['nextPositions'] = None
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _data.update({
            ...
            "processingstage": ProcessingStage.from_dict(obj["processingstage"]) if obj.get("processingstage") is not None else None,
            "nextPositions": [ProcessingProcessPositionNextPositionsInner.from_dict(_item) for _item in obj["nextPositions"]] if obj.get("nextPositions") is not None else None
        })
