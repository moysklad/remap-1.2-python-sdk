...
class CustomEntityMetadata(BaseModel):
    ...
    entity_meta: Optional[Meta] = Field(default=None, description="Метаданные ссылки на связанный пользовательский справочник (`type` — `customentity`).", alias="entityMeta")
    attributes: Optional[AttributeMetaInfoList] = None
    id: Optional[StrictStr] = Field(default=None, description="ID пользовательского справочника")
    name: Optional[Annotated[str, Field(strict=True, max_length=255)]] = Field(default=None, description="Наименование пользовательского справочника")
    create_shared: Optional[StrictBool] = Field(default=None, description="Создавать новые элементы с меткой \"Общий\"", alias="createShared")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["meta", "entityMeta", "attributes", "id", "name", "createShared"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _data.update({
            ...
            "entityMeta": Meta.from_dict(obj["entityMeta"]) if obj.get("entityMeta") is not None else None,
            "attributes": AttributeMetaInfoList.from_dict(obj["attributes"]) if obj.get("attributes") is not None else None,
            "id": obj.get("id"),
            "name": obj.get("name"),
            "createShared": obj.get("createShared")
        })
