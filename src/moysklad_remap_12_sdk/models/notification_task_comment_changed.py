...
class NotificationTaskCommentChanged(NotificationAbstract):
    """
    Уведомление об изменении комментария к задаче
    """ # noqa: E501
    diff: Optional[NotificationTaskCommentDiff] = None
    note_content: Optional[Annotated[str, Field(strict=True, max_length=4096)]] = Field(default=None, description="Содержимое комментария", alias="noteContent")
    performed_by: Optional[NotificationNamedEntity] = Field(default=None, alias="performedBy")
    task: Optional[NotificationTask] = None
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["diff", "noteContent", "performedBy", "task"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.performed_by:
            _dict['performedBy'] = self.performed_by.to_dict()
        if self.task:
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _data = dict(obj)
        _data.update({
            "diff": NotificationTaskCommentDiff.from_dict(obj["diff"]) if obj.get("diff") is not None else None,
            "noteContent": obj.get("noteContent"),
            "performedBy": NotificationNamedEntity.from_dict(obj["performedBy"]) if obj.get("performedBy") is not None else None,
            "task": NotificationTask.from_dict(obj["task"]) if obj.get("task") is not None else None
        })
